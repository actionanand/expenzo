...
export class SecurityService {
  ...
  enableBiometric(pinVerifier: string): boolean {
    if (!window.ExpenzoNative) {
      return false;
    }
    window.ExpenzoNative.enableBiometric(pinVerifier);
    return true;
  }

  requestBiometric(): void {
    window.ExpenzoNative?.authenticateBiometric();
  }

  disableBiometric(): void {
    window.ExpenzoNative?.disableBiometric();
  }

  private checkBiometricAvailability(): boolean {
    try {
      return window.ExpenzoNative?.isBiometricAvailable() ?? false;
    } catch {
      return false;
    }
  }
